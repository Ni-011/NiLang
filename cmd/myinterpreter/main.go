	...
	for i := 0; i < len(fileContentString); i++ { // for each char in the file, characterise each token
		...
		default:
			if isDigit(char) {
				...
				} else if i < len(fileContentString) && fileContentString[i] == '.' && i+1 < len(fileContentString) && !isDigit(rune(fileContentString[i+1])) { // if the dot is not followed by a digit
					fmt.Println("DOT . null")
					return
				} else if i < len(fileContentString) && fileContentString[i] == '.' && i+1 >= len(fileContentString) { // if String ends with a dot
					fmt.Println("DOT . null")
					return
				}
				...
				fmt.Println("NUMBER", output, 32, outputFloat)
			} else {
